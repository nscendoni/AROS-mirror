...
int test_mouse_serial(OOP_Class *cl, OOP_Object *o)
{
    ...
    data->shidd = OpenLibrary("serial.hidd",0);
    if (data->shidd != NULL)
    {
        data->serial = OOP_NewObject(NULL, CLID_Hidd_Serial, NULL);
        if (data->serial != NULL)
        {
            /*
           ...
           /* FIXME: Change busy loop for BOCHS!!!! */
            ...
            data->rx = AllocMem(sizeof(struct Ring), MEMF_CLEAR);
            data->mouse_inth_state = 0;  /* initialize to init state */
            ...
            if (data->rx)
            {
                for (i=0; i<4; i++)
                {
                    ...
                    data->unit = HIDD_Serial_NewUnit(data->serial, i);
                    if (data->unit != NULL)
                    {
                        int proto;
                        ...
                        HIDD_SerialUnit_Init(data->unit, mouse_RingHandler, data, NULL, NULL);

                        /* Try to get mouse protocol in PnP way */
                        if ((proto = mouse_DetectPNP(data, data->unit)) >= 0)
                        {
                            /* We got protocol */
                            data->mouse_protocol = proto;
                            switch (proto)
                            {
                                case P_MS:
                            ...
                            data->mouse_inth_state = 1;  /* initialize to event handling state */
                        ...
                        else
                        {
                            ...
                            HIDD_Serial_DisposeUnit(data->serial, data->unit);
                        }
                    }
                }
                FreeMem(data->rx, sizeof(struct Ring));
            }

            /* Found no serial mouse... Dispose serial object */
            OOP_DisposeObject(data->serial);
        }
        CloseLibrary(data->shidd);
    }
...
ULONG mouse_RingHandler(UBYTE *buf, ULONG len, ULONG unit, struct mouse_data *data)
{
    ...
    if(data->mouse_inth_state >= 1)
        handle_events(data->mouse_protocol, data);
...
//    static UBYTE inbuf[3];
    ...
    while (mouse_GetFromRing(data, &c))
    {
        ...
        data->mouse_data[data->mouse_collected_bytes++] = c;

        D(bug("mouse_data: %d, colb: %d\n", data->mouse_data[data->mouse_collected_bytes],data->mouse_collected_bytes));

        if (data->mouse_collected_bytes == 3)
        {
            data->mouse_collected_bytes = 0;
            while (!(data->mouse_data[0] & 0x40))
            {
                data->mouse_data[0] = data->mouse_data[1];
                data->mouse_data[1] = data->mouse_data[2];
...
*/
            ...
            D(bug("event 1\n"));
            e->x = (char)(((data->mouse_data[0] & 0x03) << 6) | (data->mouse_data[1] & 0x3f));
            D(bug("subevent 1\n"));
            e->y = (char)(((data->mouse_data[0] & 0x0c) << 4) | (data->mouse_data[2] & 0x3f));
            D(bug("subevent 1\n"));
            if (e->x || e->y)
            {
                D(bug("subevent 2\n"));
            ...
            buttonstate  = ((data->mouse_data[0] & 0x20) >> 5); /* left  button bit goes to bit 0 in button state */
            buttonstate |= ((data->mouse_data[0] & 0x10) >> 3); /* right button bit goes to bit 1 in button state */
...
{
    return data->rx->top - data->rx->ptr;
}
