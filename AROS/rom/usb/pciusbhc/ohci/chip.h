/*
   ...
   $Id: chip.h 45600 2012-08-19 10:26:41Z neil $
*/
...
struct HCCA
{
    ULONG ha_IntEDs[32];        /* LE PHYSICAL pointer to Interrupt EDs */
    UWORD ha_FrameCount;        /* LE Framecounter */
    UWORD ha_FrmCntChg;         /* Set 0 when framecounter was updated */
    ULONG ha_DoneHead;          /* LE PHYSICAL pointer to Head of ED finished + unmasked Int */
};
...
struct EndpointDescriptor
{
    ...
    ULONG EPCaps;               /* LE MaxPacketSize and other stuff */
    ULONG TailPtr;              /* LE PHYSICAL TD Queue Tail Pointer */
    ULONG HeadPtr;              /* LE PHYSICAL TD Queue Head Pointer */
    ULONG NextED;               /* LE PHYSICAL Next Endpoint Descriptor */
};

struct EDNode
{
    struct MinNode ed_Node;
    ULONG ed_Self;                  /* LE PHYSICAL pointer to self */
    /* On 64 bits a padding will be inserted here */
    struct IOUsbHWReq *ed_IOReq;    /* IO Request this belongs to */
    struct MinList ed_TDList;
    IPTR ed_Continue;               /* Flag for fragmented bulk transfer */
    APTR ed_Buffer;    /* Mirror buffer for data outside DMA-accessible area */
    struct UsbSetupData *ed_SetupData;    /* Mirror buffer for setup packet */
    IPTR ed_Pad[2];
    struct EndpointDescriptor ed_ED;
};

struct TransferDescriptor
{
    /* HC data, aligned to 16 bytes */
    ULONG Ctrl;                 /* LE Ctrl stuff */
    ULONG BufferPtr;            /* LE PHYSICAL Current Buffer Pointer */
    ULONG NextTD;               /* LE PHYSICAL Next TD */
    ULONG BufferEnd;            /* LE PHYSICAL End of buffer */
};

/* N.B. The ULONGs are deliberately placed apart to ensure structure size
   is a multiple of 16 bytes on 64-bit archs */
struct TDNode
{
    struct MinNode td_Node;
    ULONG td_Self;              /* LE PHYSICAL pointer to TD */
    struct EDNode *td_ED;       /* Pointer to parent ED this TD belongs to */
    ULONG td_Length;            /* Length of transfer */
    IPTR td_Pad[3];
    struct TransferDescriptor td_TD;
};
