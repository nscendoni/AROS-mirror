/*
    ...
    $Id: vmwaresvgahardware.c 55666 2019-02-05 19:27:57Z IridiumFX $
...
VOID VMWareSVGA_SetMode(struct HWData *data, ULONG width, ULONG height, ULONG depth)
{
	D(bug("[VMWareSVGA] SetMode: %dx%dx%d\n", width, height, depth));
...
{
	VMWareSVGA_SetMode(data, width, height, data->bitsperpixel);
	return;
