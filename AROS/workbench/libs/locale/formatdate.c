/*
    ...
    $Id: formatdate.c 45416 2012-08-09 03:42:46Z jmcmullan $
...
*****************************************************************************/
{
    ...
    Amiga2Date(date->ds_Days * 86400 + date->ds_Minute * 60 +
        date->ds_Tick / 50, &cData);
    ...
    while (*formatString != 0)
    {
        if (*formatString == '%')
        {
            ...
            {
            case 'a':
                _WriteString(GetLocaleStr(locale, ABDAY_1 + cData.wday),
                    hook, locale);
                break;
            ...
            case 'b':
                _WriteString(GetLocaleStr(locale,
                        ABMON_1 + cData.month - 1), hook, locale);
                break;

            case 'B':
                _WriteString(GetLocaleStr(locale, MON_1 + cData.month - 1),
                    hook, locale);
                break;
            ...
            case 'j':
                ...
                PrintDigits(cData.mday + dayspermonth[cData.month],
                    '0', 3, hook, locale);
                break;
            ...
            case 'p':
                _WriteString(GetLocaleStr(locale,
                        cData.hour < 12 ? AM_STR : PM_STR), hook, locale);
                break;
...
VOID PrintDigits(UWORD number, char fill, UWORD len,
    const struct Hook *hook, const struct Locale *locale)
{
    char buf[7];
    char *ptr = &buf[6];
    int i = 0;
