/*
    ...
    $Id: filereqsupport.c 52028 2016-03-17 04:36:20Z jmcmullan $
*/
    ...
    while ((dlist = NextDosEntry(dlist, LDF_VOLUMES | LDF_ASSIGNS)) != NULL)
    {
        ...
        if ((node = AllocPooled(ld->ld_IntReq->ir_MemPool, sizeof(struct ASLLVFileReqNode))))
        {
            if (dlist->dol_Name)
            {
                ...
                ULONG len = AROS_BSTR_strlen(dlist->dol_Name);

                node->text[0] = PooledCloneStringLen(name, len, ":", 1, ld->ld_IntReq->ir_MemPool, AslBase);
