/* 
    ...
    $Id: bubbleengine.c 45683 2012-08-29 16:17:40Z neil $
*/
...
#define WINHEIGHT \
    ((bubble->ztext->height  + BORDER_Y * 2) < ROUNDEDGE_SIZE * 2 + 1) ? \
...
static struct Region *zune_bubble_shape_create(Object *obj,
    ZBubble *bubble, WORD w, WORD h)
{
...
APTR zune_bubble_create(Object *obj, LONG x, LONG y, char *text,
    ULONG flags)
{
    ZBubble *bubble;
    ...
    bubble->shape =
        zune_bubble_shape_create(obj, bubble, WINWIDTH, WINHEIGHT);
...
void zune_bubble_delete(Object *obj, APTR bubble)
{
    ...
    if (b)
    {
        if (b->win)
            CloseWindow(b->win);
        if (b->shape)
            DisposeRegion(b->shape);
        if (b->ztext)
            zune_text_destroy(b->ztext);
