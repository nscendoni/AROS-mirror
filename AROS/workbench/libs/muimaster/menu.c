    ...
    WORD *xmin, WORD *ymin, WORD *xmax, WORD *ymax)
{
    ...
    if (xmin)
        *xmin = left - win->WScreen->MenuHBorder;
    if (ymin)
        *ymin = top - win->WScreen->MenuVBorder;
    if (xmax)
        *xmax = right + win->WScreen->MenuHBorder;
    if (ymax)
        *ymax = bottom + win->WScreen->MenuVBorder;
...
void CalculateDims(struct Window *win, struct Menu *menu)
{
    ...
    while (menu != NULL)
    {
        ...
        GetMenuBox(win, item, &menu->JazzX, &menu->JazzY, &menu->BeatX,
            &menu->BeatY);
...
static void HandleCheckItem(struct Window *win, struct MenuItem *item,
    WORD itemnum, struct MenuHandlerData *mhd);

static void HighlightMenuTitle(struct Menu *menu,
...
static struct MenuItem *FindSubItem(WORD *var,
    struct MenuHandlerData *mhd);
...
static void RenderItem(struct MenuItem *item, WORD itemtype,
    struct Rectangle *box, struct MenuHandlerData *mhd);

static void RenderMenuBG(struct Window *win, struct MenuHandlerData *mhd);
static void RenderCheckMark(struct MenuItem *item, WORD itemtype,
    struct MenuHandlerData *mhd);
static void RenderAmigaKey(struct MenuItem *item, WORD itemtype,
    struct MenuHandlerData *mhd);
static void RenderDisabledPattern(struct RastPort *rp, WORD x1, WORD y1,
    WORD x2, WORD y2, struct MenuHandlerData *mhd);
#if 0
static void RenderFrame(struct RastPort *rp, WORD x1, WORD y1, WORD x2,
    WORD y2, WORD state, struct MenuHandlerData *mhd);
#endif
static void HighlightItem(struct MenuItem *item, WORD itemtype,
    struct MenuHandlerData *mhd);
static WORD CalcMaxCommKeyWidth(struct Window *win,
    struct MenuHandlerData *mhd);
    ...
    WORD itemnum, struct MenuHandlerData *mhd)
{
    ...
    if (re_render)
    {
        ...
        if ((item->Flags & HIGHITEM) &&
            ((item->Flags & HIGHFLAGS) == HIGHCOMP))
            toggle_hi = TRUE;

        if (toggle_hi)
            HighlightItem(item, itemtype, mhd);
    ...
    {
        struct MenuItem *checkitem =
            (itemtype ==
            ITEM_ITEM) ? mhd->activemenu->FirstItem : mhd->activeitem->
            SubItem;
        BOOL toggle_hi = FALSE;
        WORD i;

        if ((item->Flags & HIGHITEM) &&
            ((item->Flags & HIGHFLAGS) == HIGHBOX))
            toggle_hi = TRUE;

        if (toggle_hi)
            HighlightItem(item, itemtype, mhd);

        for (i = 0; (i < 32) && checkitem;
            i++, checkitem = checkitem->NextItem)
        {
            if ((i != itemnum) && (item->MutualExclude & (1L << i)) &&
                ((checkitem->Flags & (CHECKED | CHECKIT)) ==
                    (CHECKIT | CHECKED)))
            {
    ...
    struct MenuHandlerData *mhd)
{
    if (menu->Flags & MENUENABLED)
    {
...
#else
        WORD x1 =
            menu->LeftEdge + mhd->scr->BarHBorder - mhd->scr->MenuHBorder;
        WORD y1 = 0;
        ...
        SetDrMd(rp, JAM1);
        SetAPen(rp,
            mhd->dri->dri_Pens[(menu->
                    Flags & HIGHITEM) ? FILLPEN : BACKGROUNDPEN]);
        RectFill(rp, x1, y1, x2, y2);
...
static struct Menu *FindMenu(WORD *var, struct MenuHandlerData *mhd)
{
    ...
    if ((mouse_x >= 0) && (mouse_x < mhd->menubaritemwidth)
        && (mouse_y >= 0))
    {
    ...
    for (menu = mhd->menu, i = 0; menu; menu = menu->NextMenu, i++)
    {
        if ((mouse_x >= menu->LeftEdge) &&
            ...
            (mouse_y >= 0) && (mouse_y <= mhd->scr->BarHeight))
        {
            *var = i;
            break;
        }
...
static struct MenuItem *FindItem(WORD *var, struct MenuHandlerData *mhd)
{
    ...
    {
        mouse_x =
            mhd->scrmousex - mhd->menuwin->LeftEdge +
            mhd->activemenu->JazzX;
        mouse_y =
            mhd->scrmousey - mhd->menuwin->TopEdge + mhd->activemenu->JazzY;

        for (item = mhd->activemenu->FirstItem, i = 0; item;
            item = item->NextItem, i++)
        {
...
static struct MenuItem *FindSubItem(WORD *var, struct MenuHandlerData *mhd)
{
    ...
    {
        mouse_x =
            mhd->scrmousex - mhd->submenuwin->LeftEdge +
            mhd->submenubox.MinX;
        mouse_y =
            mhd->scrmousey - mhd->submenuwin->TopEdge +
            mhd->submenubox.MinY;

        *var = -1;

        for (item = mhd->activeitem->SubItem, i = 0; item;
            item = item->NextItem, i++)
        {
            if ((mouse_x >= item->LeftEdge) &&
                (mouse_x < item->LeftEdge + item->Width) &&
...
static void MakeMenuBarWin(struct MenuHandlerData *mhd)
{
    ...
    {
        w = TextLength(&mhd->scr->RastPort, menu->MenuName,
    ...
    mhd->menubaritemwidth =
        maxw + TextLength(&mhd->scr->RastPort, subitemindicator,
        1) + TEXT_AMIGAKEY_SPACING + ITEXT_EXTRA_LEFT + ITEXT_EXTRA_RIGHT;

    mhd->menubaritemheight =
        mhd->scr->RastPort.TxHeight + ITEXT_EXTRA_TOP + ITEXT_EXTRA_BOTTOM;

    win_tags[2].ti_Data = mhd->menubaritemwidth + mhd->scr->MenuHBorder * 2;
    win_tags[3].ti_Data =
        mhd->menubaritemheight * mhd->nummenubaritems +
        mhd->scr->MenuVBorder * 2;
...
static void RenderMenuBar(struct MenuHandlerData *mhd)
{
    if (mhd->menubarwin)
    {
...
#endif
        RectFill(rp, 0, 0, mhd->menubarwin->Width - 1,
            mhd->menubarwin->Height - 2);
        SetAPen(rp, mhd->dri->dri_Pens[BARTRIMPEN]);
        RectFill(rp, 0, mhd->menubarwin->Height - 1,
            mhd->menubarwin->Width - 1, mhd->menubarwin->Height - 1);
        ...
        SetAPen(rp, mhd->dri->dri_Pens[SHADOWPEN]);
        RectFill(rp, mhd->menubarwin->Width - 1, 1,
            mhd->menubarwin->Width - 1, mhd->menubarwin->Height - 2);
...
static void RenderMenuTitle(struct Menu *menu, struct MenuHandlerData *mhd)
{
    ...
    y = mhd->scr->MenuVBorder + ITEXT_EXTRA_TOP +
        yoff * mhd->menubaritemheight;
#else
    ...
    SetAPen(rp,
        mhd->dri->dri_Pens[(menu->
                Flags & HIGHITEM) ? FILLTEXTPEN : TEXTPEN]);
#endif
    ...
    if (menu->FirstItem)
    {
        ...
        WORD x2 =
            mhd->scr->MenuHBorder + mhd->menubaritemwidth -
            ITEXT_EXTRA_RIGHT - silen;
...
#else
    WORD xpos =
        mhd->activemenu->LeftEdge + mhd->scr->BarHBorder +
        mhd->activemenu->JazzX;
...
#if MENUS_UNDERMOUSE
    win_tags[1].ti_Data +=
        (mhd->menubaritemheight * mhd->activemenunum +
        mhd->scr->MenuVBorder) - height / 2;
...
static void RenderMenu(struct MenuHandlerData *mhd)
{
    ...
    if (mhd->menuwin)
    {
        ...
        for (item = mhd->activemenu->FirstItem; item; item = item->NextItem)
        {
            RenderItem(item, ITEM_ITEM,
                (struct Rectangle *)(&mhd->activemenu->JazzX), mhd);
        }
    }
}
...
static void MakeSubMenuWin(struct MenuHandlerData *mhd)
{
    ...
    GetMenuBox(mhd->menubarwin, item, &mhd->submenubox.MinX,
        &mhd->submenubox.MinY,
        &mhd->submenubox.MaxX, &mhd->submenubox.MaxY);
    ...
    mhd->maxcommkeywidth_submenu =
        CalcMaxCommKeyWidth(mhd->submenuwin, mhd);
...
static void RenderSubMenu(struct MenuHandlerData *mhd)
{
    ...
    if (mhd->submenuwin)
    {
        ...
        {
            RenderItem(item, ITEM_SUBITEM,
                (struct Rectangle *)(&mhd->submenubox), mhd);
        }
    }
}
...
{
    struct Window *win =
        ((itemtype == ITEM_ITEM) ? mhd->menuwin : mhd->submenuwin);
    ...
    BOOL enabled = ((item->Flags & ITEMENABLED) &&
        ...
        ((itemtype == ITEM_ITEM)
            || (mhd->activeitem->Flags & ITEMENABLED)));
    ...
    if (item->ItemFill)
    {
        ...
        else
        {
            ...
            DrawImageState(rp, im, offx + item->LeftEdge,
                offy + item->TopEdge, state, mhd->dri);
        }
    ...
    if (!enabled && !item_supports_disable)
    {
        RenderDisabledPattern(rp, offx + item->LeftEdge,
            ...
            offy + item->TopEdge + item->Height - 1, mhd);
    }
...
static void RenderMenuBG(struct Window *win, struct MenuHandlerData *mhd)
{
...
#endif
    RectFill(rp, borderx,
        bordery, win->Width - 1 - borderx, win->Height - 1 - bordery);
    ...
    RectFill(rp, win->Width - borderx, bordery, win->Width - 1,
        win->Height - 1);
    RectFill(rp, 0, win->Height - bordery, win->Width - 1 - borderx,
        win->Height - 1);
#else
    ...
    struct MenuHandlerData *mhd)
{
    ...
    struct Rectangle *box =
        ((itemtype ==
            ITEM_ITEM) ? ((struct Rectangle *)&mhd->activemenu->
            JazzX) : &mhd->submenubox);
    WORD offx = -box->MinX;
    WORD offy = -box->MinY;
    WORD state = ((item->Flags & HIGHITEM) &&
        ((item->Flags & HIGHFLAGS) ==
            HIGHCOMP)) ? IDS_SELECTED : IDS_NORMAL;
    ...
    if (item->Flags & CHECKIT)
    {
        ...
        y1 = item->TopEdge + offy + (item->Height -
            mhd->checkmark->Height) / 2;
...
#else
            SetAPen(rp,
                mhd->dri->dri_Pens[(state ==
                        IDS_SELECTED) ? FILLPEN : BACKGROUNDPEN]);
#endif
            RectFill(rp, x1, y1, x2, y2);
        }
    }
    ...
    struct MenuHandlerData *mhd)
{
    ...
    WORD commkeywidth =
        ((itemtype ==
            ITEM_ITEM) ? mhd->maxcommkeywidth_menu : mhd->
    ...
    if (item->Flags & COMMSEQ)
    {
        ...
        y1 = item->TopEdge + offy + (item->Height - mhd->amigakey->Height +
            1) / 2;
...
#endif
        Move(rp, x1,
            item->TopEdge + offy + (item->Height - rp->TxHeight) / 2 +
            rp->TxBaseline);
...
{
    SetAPen(rp,
        mhd->dri->dri_Pens[(state == IDS_SELECTED) ? SHADOWPEN : SHINEPEN]);

    RectFill(rp, x1, y1, x2, y1);
    RectFill(rp, x1, y1 + 1, x1, y2);

    SetAPen(rp,
        mhd->dri->dri_Pens[(state == IDS_SELECTED) ? SHINEPEN : SHADOWPEN]);
    ...
    struct MenuHandlerData *mhd)
{
    ...
    if (!(mhd->activemenu->Flags & MENUENABLED))
        enabled = FALSE;
    if ((itemtype == ITEM_SUBITEM)
        && !(mhd->activeitem->Flags & ITEMENABLED))
        enabled = FALSE;
    ...
    if (enabled)
    {
        ...
        fill = item->ItemFill;
        if ((item->Flags & HIGHITEM) && (item->SelectFill))
        ...
        case HIGHIMAGE:
            ...
            else
            {
                ...
                DrawImageState(rp, (struct Image *)fill, x1, y1,
                    IDS_SELECTED, mhd->dri);
            }
            break;
        ...
        case HIGHCOMP:
            ...
            {
                WORD state =
                    (item->Flags & HIGHITEM) ? IDS_SELECTED : IDS_NORMAL;
...
static void AddToSelection(struct MenuHandlerData *mhd)
{
        ...
        && (mhd->activeitem->Flags & ITEMENABLED))
    {
        struct MenuItem *item = NULL;
        UWORD men =
            FULLMENUNUM(mhd->activemenunum, mhd->activeitemnum,
            mhd->activesubitemnum);

        if (mhd->activesubitemnum != -1)
        {
            if (mhd->activesubitem->Flags & ITEMENABLED)
                item = mhd->activesubitem;
        }
            ...
            else if (men != mhd->lastmenupick)
            {
                ...
                while (checkmen != MENUNULL)
                {
                    ...
                    if (checkmen == men)
                    {
                        ...
                        {
                            prevcheckitem->NextSelect =
                                checkitem->NextSelect;
                        }
                    }
