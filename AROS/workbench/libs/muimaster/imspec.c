...
static struct MUI_ImageSpec_intern *get_pattern_imspec(LONG in)
{
    ...
    if (in >= MUII_BACKGROUND && in <= MUII_FILL)
    {
        ...
        {
            UWORD color;
            if (in == MUII_BACKGROUND)
                color = MPEN_BACKGROUND;
            else if (in == MUII_SHADOW)
                color = MPEN_SHADOW;
            else if (in == MUII_SHINE)
                color = MPEN_SHINE;
            else
                color = MPEN_FILL;
...
static struct MUI_ImageSpec_intern *get_scaled_gradient_imspec(CONST_STRPTR
    str)
{
    struct MUI_ImageSpec_intern *spec;
...
static struct MUI_ImageSpec_intern *get_tiled_gradient_imspec(CONST_STRPTR
    str)
{
    struct MUI_ImageSpec_intern *spec;
...
static const char *zune_imspec_to_string(struct MUI_ImageSpec_intern *spec)
{
    ...
    case IST_COLOR:
        zune_pen_intern_to_spec(&spec->u.penspec,
            (struct MUI_PenSpec *)buf);
        break;
...
#if 0
static struct MUI_ImageSpec_intern *zune_imspec_copy(struct
    MUI_ImageSpec_intern *spec)
{
    ...
    if (nspec)
        memcpy(nspec, spec, sizeof(struct MUI_ImageSpec_intern));
    return nspec;
}
#endif
    ...
    case IST_BRUSH:
        {
            ...
            for (i = 0; i < 2; i++)
            {
                if (spec->u.brush.filename[i])
                {
                    ...
                    if (!spec->u.brush.dt[i]
                        && !strchr(spec->u.brush.filename[i], ':'))
                    {
                        int size;
                        STRPTR fullpath;
...
void zune_imspec_drawbuffered(struct MUI_ImageSpec_intern *spec,
    struct RastPort *rp, struct MUI_RenderInfo *mri, LONG left, LONG top,
    LONG width, LONG height, LONG xoffset, LONG yoffset, LONG state,
    LONG dx, LONG dy, WORD mode, LONG abs_l, LONG abs_t, LONG abs_r,
    LONG abs_b)
{
    ...
    case IST_COLOR:
        zune_penspec_drawdirect(&spec->u.penspec, rp, mri, left - dx,
            top - dy, right - dx, bottom - dy);
        break;
    ...
    case IST_SCALED_GRADIENT:
        if (mode == 0)
            ...
            zune_gradient_drawfast(spec, rp, mri, 1, abs_l, abs_t, abs_r,
                abs_b, left, top, right, bottom, xoffset, yoffset);
        break;

    case IST_TILED_GRADIENT:
        if (mode == 0)
            zune_gradient_draw(spec, mri, left, top, right, bottom, xoffset,
                yoffset);
        else
            zune_gradient_drawfast(spec, rp, mri, 1, abs_l, abs_t, abs_r,
                abs_b, left, top, right, bottom, xoffset, yoffset);
...
void zune_imspec_draw(struct MUI_ImageSpec_intern *spec,
