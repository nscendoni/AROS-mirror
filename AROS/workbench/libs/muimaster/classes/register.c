/*
    ...
    $Id: register.c 45798 2012-09-14 19:47:19Z neil $
*/
...
static void LayoutTabItems(Object *obj, struct Register_DATA *data)
{
    ...
    item_width = (_width(obj) - data->total_hspacing) / data->columns;
        //data->numitems;
...
/**************************************************************************
   Render one item
**************************************************************************/
static void RenderRegisterTabItem(struct IClass *cl, Object *obj,
    WORD item)
{
    ...
    RectFill(_rp(obj), left_item_bar_x + 4, top_item_bar_y,
        right_item_bar_x - 4, top_item_bar_y);
    /* left vert bar */
    RectFill(_rp(obj), left_item_bar_x, top_item_bar_y + 4, left_item_bar_x,
        bottom_item_bar_y);
    ...
    RectFill(_rp(obj), right_item_bar_x, top_item_bar_y + 4,
        right_item_bar_x, bottom_item_bar_y);
 ...
 */
    ...
    else
    {
        ...
        if (data->oldactive >= data->columns)
            method = MUIM_DrawBackground;
        else
            method = MUIM_DrawParentBackground;
        ...
        RectFill(_rp(obj), old_left, old_top + old_height,
            old_left + old_width, old_top + old_height);
    }
 ...
 */
    /* register frame */
    if (flags & MADF_DRAWOBJECT
        || (data->active / data->columns !=
            data->oldactive / data->columns))
    {
        ...
        for (i = 0; i < data->rows; i++)
        {
            ...
            {
                y = _bottom(obj) - muiAreaData(obj)->mad_InnerBottom +
                    data->tab_height;
                tabs_on_bottom = 1;
            }

            RectFill(_rp(obj), data->left + 1, y,
                data->left + data->framewidth - 2, y);
            y += data->tab_height;
        }
...
static void SetHardCoord(Object *obj, struct Register_DATA *data)
{
    ...
    //adata->mad_InnerTop = data->tab_height
    //    * (1 + data->active/data->columns) + REGISTER_FRAMETOP;
    //adata->mad_InnerRight = REGISTER_FRAMEX;
    //adata->mad_InnerBottom = data->tab_height
    ...
    adata->mad_InnerTop =
        data->tab_height * (1 + data->active / data->columns) +
        frame->innerTop;
...
IPTR Register__OM_NEW(struct IClass *cl, Object *obj, struct opSet *msg)
{
    ...
    obj =
        (Object *) DoSuperNewTags(cl, obj, NULL, MUIA_Group_PageMode, TRUE,
        MUIA_Background, MUII_RegisterBack, TAG_MORE,
    ...
    data->labels =
        (char **)GetTagData(MUIA_Register_Titles, 0, msg->ops_AttrList);
    ...
    data->columns =
        (WORD) GetTagData(MUIA_Register_Columns, data->numitems,
        msg->ops_AttrList);
    if (data->columns <= 0)
        data->columns = 1;
    ...
    data->items =
        (struct RegisterTabItem *)AllocVec(data->numitems *
        sizeof(struct RegisterTabItem), MEMF_PUBLIC | MEMF_CLEAR);
...
IPTR Register__MUIM_Setup(struct IClass *cl, Object *obj,
...
/*    D(bug("Register_AskMinMax : data->total_hspacing = %d\n",
    ...
    muiAreaData(obj)->mad_Flags |=
        (MADF_INNERLEFT | MADF_INNERTOP | MADF_INNERRIGHT |
        MADF_INNERBOTTOM);
...
IPTR Register__MUIM_Cleanup(struct IClass *cl, Object *obj,
...
IPTR Register__MUIM_AskMinMax(struct IClass *cl, Object *obj,
...
/*          "mymin=%d, mydef=%d\n", */
/*          data->total_hspacing, msg->MinMaxInfo->MinWidth, */
/*          msg->MinMaxInfo->DefWidth, */
/*          data->min_width, data->def_width)); */

    msg->MinMaxInfo->MinWidth =
        MAX(msg->MinMaxInfo->MinWidth, data->min_width);
    msg->MinMaxInfo->MaxWidth =
        MAX(msg->MinMaxInfo->MaxWidth, data->def_width);
    msg->MinMaxInfo->DefWidth =
        MAX(msg->MinMaxInfo->DefWidth, data->def_width);
...
IPTR Register__MUIM_Layout(struct IClass *cl, Object *obj,
...
/*    D(bug( */
/*    "Register_Layout : left=%d, top=%d / framewidth=%d, frameheight=%d\n", */
/*        data->left, data->top, data->framewidth, data->frameheight)); */

    return retval;
}

IPTR Register__MUIM_Show(struct IClass *cl, Object *obj,
...
IPTR Register__MUIM_Draw(struct IClass *cl, Object *obj,
    struct MUIP_Draw *msg)
{
     ...
     * so on */
    if (data->oldactive != data->active && (msg->flags & MADF_DRAWUPDATE)
        && (data->active / data->columns !=
            data->oldactive / data->columns))
    {
...
IPTR Register__MUIM_HandleEvent(struct IClass *cl, Object *obj,
...
BOOPSI_DISPATCHER(IPTR, Register_Dispatcher, cl, obj, msg)
{
    ...
    {
    case OM_NEW:
        return Register__OM_NEW(cl, obj, (struct opSet *)msg);
    case OM_GET:
        return Register__OM_GET(cl, obj, (struct opGet *)msg);
    case OM_DISPOSE:
        return Register__OM_DISPOSE(cl, obj, msg);
    case MUIM_Setup:
        return Register__MUIM_Setup(cl, obj, (struct MUIP_Setup *)msg);
    case MUIM_Cleanup:
        return Register__MUIM_Cleanup(cl, obj, (struct MUIP_Cleanup *)msg);
    case MUIM_AskMinMax:
        return Register__MUIM_AskMinMax(cl, obj,
            (struct MUIP_AskMinMax *)msg);
    case MUIM_Layout:
        return Register__MUIM_Layout(cl, obj, (struct MUIP_Layout *)msg);
    case MUIM_Show:
        return Register__MUIM_Show(cl, obj, (struct MUIP_Show *)msg);
    case MUIM_Draw:
        return Register__MUIM_Draw(cl, obj, (struct MUIP_Draw *)msg);
    case MUIM_HandleEvent:
        return Register__MUIM_HandleEvent(cl, obj,
            (struct MUIP_HandleEvent *)msg);
    default:
        return DoSuperMethodA(cl, obj, msg);
    }
