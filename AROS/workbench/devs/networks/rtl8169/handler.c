/*
 * $Id: handler.c 44683 2012-04-23 18:38:57Z jmcmullan $
 */
...
void handle_request(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    ReleaseSemaphore(&((struct RTL8169Unit *)request->ios2_Req.io_Unit)->rtl8169u_unit_lock);
}
...
static BOOL CmdWrite(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    {
		request->ios2_Req.io_Flags &= ~IOF_QUICK;
		PutMsg(unit->rtl8169u_request_ports[WRITE_QUEUE], (APTR)request);
    }
    else
    {
...
static BOOL CmdS2DeviceQuery(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    CopyMem(&unit->rtl8169u_Sana2Info, info, size);

    info->BPS = unit->rtl8169u_rtl_LinkSpeed;
    info->MTU = ETH_MTU;
...
static BOOL CmdGetStationAddress(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    CopyMem(unit->rtl8169u_dev_addr, request->ios2_SrcAddr, ETH_ADDRESSSIZE);
    CopyMem(unit->rtl8169u_org_addr, request->ios2_DstAddr, ETH_ADDRESSSIZE);
...
static BOOL CmdConfigInterface(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    if((unit->rtl8169u_flags & IFF_CONFIGURED) == 0)
    {
	    CopyMem(request->ios2_SrcAddr, unit->rtl8169u_dev_addr, ETH_ADDRESSSIZE);
	    unit->set_mac_address(unit);
	    unit->rtl8169u_flags |= IFF_CONFIGURED;
    }
    else
    {
...
static BOOL CmdTrackType(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
	...
	FindTypeStats(LIBBASE, unit, &unit->rtl8169u_type_trackers, packet_type);
    ...
    {
		tracker = AllocMem(sizeof(struct TypeTracker), MEMF_PUBLIC | MEMF_CLEAR);
		if(tracker != NULL)
		{
		    ...
		    Disable();
		    AddTail((APTR)&unit->rtl8169u_type_trackers, (APTR)tracker);
		    Enable();
		}
	}
...
static BOOL CmdGetGlobalStats(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    CopyMem(&unit->rtl8169u_stats, request->ios2_StatData,
...
static BOOL CmdOnEvent(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    {
		request->ios2_Req.io_Flags &= ~IOF_QUICK;
		PutMsg(unit->rtl8169u_request_ports[EVENT_QUEUE], (APTR)request);
    }
...
static BOOL CmdReadOrphan(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    ...
    {
		request->ios2_Req.io_Flags &= ~IOF_QUICK;
		PutMsg(unit->rtl8169u_request_ports[ADOPT_QUEUE], (struct Message *) request);
    }
    else
    {
		request->ios2_Req.io_Error = error;
...
static BOOL CmdOnline(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    struct RTL8169Unit *unit = (struct RTL8169Unit *)request->ios2_Req.io_Unit;
    BYTE error = 0;
    ULONG wire_error = 0;
    UWORD i;
    ...
    if((error == 0) && ((unit->rtl8169u_flags & IFF_UP) == 0))
    {
		unit->rtl8169u_stats.PacketsReceived = 0;
		unit->rtl8169u_stats.PacketsSent = 0;
		unit->rtl8169u_stats.BadData = 0;
		unit->rtl8169u_stats.Overruns = 0;
		unit->rtl8169u_stats.UnknownTypesReceived = 0;
		unit->rtl8169u_stats.Reconfigurations = 0;

		for(i = 0; i < STAT_COUNT; i++)
		{
		    unit->rtl8169u_special_stats[i] = 0;
		}
