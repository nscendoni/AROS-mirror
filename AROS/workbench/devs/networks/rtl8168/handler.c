...
RTLD(bug("[%s] S2CmdRead()\n", unit->rtl8168u_name))
    ...
    if((unit->rtl8168u_flags & IFF_UP) != 0)
    {
...
RTLD(bug("[%s] S2CmdWrite()\n", unit->rtl8168u_name))
    ...
    if(error == 0) {
	request->ios2_Req.io_Flags &= ~IOF_QUICK;
	PutMsg(unit->rtl8168u_request_ports[WRITE_QUEUE], (APTR)request);
    }
    else
    {
...
RTLD(bug("[%s] S2CmdDeviceQuery()\n", unit->rtl8168u_name))
    ...
    CopyMem(&unit->rtl8168u_Sana2Info, info, size);

    info->BPS = unit->rtl8168u_rtl_LinkSpeed;
    info->MTU = ETH_MTU;
...
RTLD(bug("[%s] S2CmdGetStationAddress()\n", unit->rtl8168u_name))
    ...
    CopyMem(unit->rtl8168u_dev_addr, request->ios2_SrcAddr, ETH_ADDRESSSIZE);
    CopyMem(unit->rtl8168u_org_addr, request->ios2_DstAddr, ETH_ADDRESSSIZE);
...
RTLD(bug("[%s] S2CmdConfigInterface()\n", unit->rtl8168u_name))
    ...
    if((unit->rtl8168u_flags & IFF_CONFIGURED) == 0)
    {
	CopyMem(request->ios2_SrcAddr, unit->rtl8168u_dev_addr, ETH_ADDRESSSIZE);
	unit->set_mac_address(unit);
	unit->rtl8168u_flags |= IFF_CONFIGURED;
    }
    else
    {
...
RTLD(bug("[%s] S2CmdTrackType()\n", unit->rtl8168u_name))
	...
	FindTypeStats(LIBBASE, unit, &unit->rtl8168u_type_trackers, packet_type);
	...
	if(tracker != NULL)
	{
	    ...
	    Disable();
	    AddTail((APTR)&unit->rtl8168u_type_trackers, (APTR)tracker);
	    Enable();
	}
   }
...
RTLD(bug("[%s] S2CmdGetGlobalStats()\n", unit->rtl8168u_name))
    ...
    CopyMem(&unit->rtl8168u_stats, request->ios2_StatData,
...
RTLD(bug("[%s] S2CmdOnEvent()\n", unit->rtl8168u_name))
    ...
    {
	request->ios2_Req.io_Flags &= ~IOF_QUICK;
	PutMsg(unit->rtl8168u_request_ports[EVENT_QUEUE], (APTR)request);
    }
...
RTLD(bug("[%s] S2CmdReadOrphan()\n", unit->rtl8168u_name))
    ...
    {
	request->ios2_Req.io_Flags &= ~IOF_QUICK;
	PutMsg(unit->rtl8168u_request_ports[ADOPT_QUEUE], (struct Message *)request);
    }
    else
    {
	request->ios2_Req.io_Error = error;
...
static BOOL CmdOnline(LIBBASETYPEPTR LIBBASE, struct IOSana2Req *request)
{
    struct RTL8168Unit *unit = (struct RTL8168Unit *)request->ios2_Req.io_Unit;
    BYTE error = 0;
    ULONG wire_error = 0;
    UWORD i;
...
RTLD(bug("[%s] S2CmdOnline()\n", unit->rtl8168u_name))
    ...
    if((error == 0) && ((unit->rtl8168u_flags & IFF_UP) == 0))
    {
	unit->rtl8168u_stats.PacketsReceived = 0;
	unit->rtl8168u_stats.PacketsSent = 0;
	unit->rtl8168u_stats.BadData = 0;
	unit->rtl8168u_stats.Overruns = 0;
	unit->rtl8168u_stats.UnknownTypesReceived = 0;
	unit->rtl8168u_stats.Reconfigurations = 0;

	for(i = 0; i < STAT_COUNT; i++)
	    unit->rtl8168u_special_stats[i] = 0;
