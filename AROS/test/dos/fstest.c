/*
    ...
    $Id: fstest.c 51315 2016-01-17 23:41:58Z neil $
*/
...
int deleteDirContents(char *startpath, int depth, int dnum, int fnum)
{
    ...
    while (next != NULL)
    {
        ...
        {
            if (deleteDirContents(dpath, depth - 1, dnum - 1,
                fnum == 0 ? fnum : fnum - 1) != 0)
            {
                FreeDosObject(DOS_EXALLCONTROL, eac);
                FreeVec(ead);
                return 1;
...
int checkParent(char *startpath, int depth, int dnum, int fnum, int size)
{
    ...
    for (i = 0; i < dnum; i++)
    {
        ...
        if (depth > 0)
            if (checkParent(path, depth - 1, dnum - 1,
                fnum == 0 ? fnum : fnum - 1, size) != 0)
                return 1;
    }
    UnLock(lock);
    return 0;
...
int checkFile(char *path, int depth, int dnum, int fnum, int size)
{
    ...
    for (i = 0; i < (size / 512); i++)
    {
        ...
        for (j = 0; j < (512 >> 4); j += 4)
        {
            if ((buffer[j + 0] != depth) ||
                (buffer[j + 1] != dnum) ||
                (buffer[j + 2] != fnum) || (buffer[j + 3] != size))
            {
...
int checkFiles(char *startpath, int depth, int dnum, int fnum, int size)
{
    ...
    for (i = 0; i < dnum; i++)
    {
        ...
        if (depth > 0)
            if (checkFiles(path, depth - 1, dnum - 1,
                    fnum == 0 ? fnum : fnum - 1, size) != 0)
                return 1;
    }
    return 0;
...
int createFiles(char *startpath, int depth, int dnum, int fnum, int size)
{
    ...
    for (i = 0; i < dnum; i++)
    {
        ...
        if (depth > 0)
            if (createFiles(path, depth - 1, dnum - 1,
                fnum == 0 ? fnum : fnum - 1, size) != 0)
                return 1;
    }
    return 0;
...
{
    printf("Verifying %d files per depth with size of %d bytes ...", fnum,
        size);
...
int fileTest(char *startpath, int depth, int dnum, int fnum, int isize)
{
    ...
    for (i = 0; i < 5; i++)
    {
        printf("Creating %d files per depth with size of %d bytes ...",
            fnum, size);
