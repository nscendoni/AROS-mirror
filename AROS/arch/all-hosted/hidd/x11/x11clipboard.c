/*
    ...
    $Id: x11clipboard.c 53031 2016-10-25 16:45:22Z deadwood $
...
STATIC VOID listen_for_xevent(struct x11_staticdata *xsd, long event, BOOL yesno)
{
    ...
    LOCK_X11
    XCALL(XGetWindowAttributes, xsd->display,
    ...
    XCALL(XSelectInput, xsd->display, xsd->dummy_window_for_creating_pixmaps, event);
    UNLOCK_X11
}
...
VOID x11clipboard_handle_event(struct x11_staticdata *xsd, XEvent *event)
{
	...
	case SelectionRequest:
		...
		else if (event->xselectionrequest.target == XA_STRING)
		{
		    ...
		    LOCK_X11
		    xsd->hostclipboard_write_chunks = XCALL(XMaxRequestSize, xsd->display) * 4 / 2;
