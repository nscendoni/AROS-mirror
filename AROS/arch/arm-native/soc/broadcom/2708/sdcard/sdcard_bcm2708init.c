...
static int FNAME_BCMSDC(BCM2708Init)(struct SDCardBase *SDCardBase)
{
    ...
    MBoxMessage[0] = 8 * 4;
    MBoxMessage[1] = VCTAG_REQ;
    MBoxMessage[2] = VCTAG_GETPOWER;
    MBoxMessage[3] = 8;
    MBoxMessage[4] = 4;
    MBoxMessage[5] = VCPOWER_SDHCI;
    MBoxMessage[6] = 0;
    ...
    if (!(MBoxMessage[6] & VCPOWER_STATE_ON))
    {
        DINIT(bug("[SDCard--] %s: Powering on Arasan SDHCI controller...\n", __PRETTY_FUNCTION__));

        MBoxMessage[0] = 8 * 4;
        MBoxMessage[1] = VCTAG_REQ;
        MBoxMessage[2] = VCTAG_SETPOWER;
        MBoxMessage[3] = 8;
        MBoxMessage[4] = 8;
        MBoxMessage[5] = VCPOWER_SDHCI;
        MBoxMessage[6] = VCPOWER_STATE_ON | VCPOWER_STATE_WAIT;

        MBoxMessage[7] = 0; // terminate tag

        MBoxWrite((APTR)VCMB_BASE, VCMB_PROPCHAN, MBoxMessage);
        if ((MBoxRead((APTR)VCMB_BASE, VCMB_PROPCHAN) != MBoxMessage) || (!(MBoxMessage[6] & VCPOWER_STATE_ON)))
        {
    ...
    MBoxMessage[0] = 8 * 4;
    MBoxMessage[1] = VCTAG_REQ;
    MBoxMessage[2] = VCTAG_GETCLKRATE;
    MBoxMessage[3] = 8;
    MBoxMessage[4] = 4;
    MBoxMessage[5] = VCCLOCK_SDHCI;
    MBoxMessage[6] = 0;
    ...
    if ((__BCM2708Bus = AllocPooled(SDCardBase->sdcard_MemPool, sizeof(struct sdcard_Bus))) != NULL)
    {
        ...
        __BCM2708Bus->sdcb_ClockMax = MBoxMessage[6];
        __BCM2708Bus->sdcb_ClockMin = BCM2708SDCLOCK_MIN;        

        __BCM2708Bus->sdcb_LEDCtrl = FNAME_BCMSDCBUS(BCMLEDCtrl);
